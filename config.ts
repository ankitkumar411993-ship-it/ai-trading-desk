...
export const config = {
  ...
  // Powers the "chat with an employee" feature — each employee (Trend Analyst, Liquidity
  // Analyst, Risk Manager, Portfolio Manager, CEO) can answer questions about their own current
  // analysis. Their answers are grounded in the real numbers from the latest scan cycle (see
  // src/services/employeeChat.ts), not freely generated, so they can't contradict what's
  // actually on screen. Get a key at https://console.anthropic.com — chat is simply disabled
  // (with a clear message in the UI) if this isn't set.
  anthropic: {
