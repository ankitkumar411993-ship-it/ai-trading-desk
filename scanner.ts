...
import { updateLatestScanState } from "../state/latestScanState";
...
export class Scanner {
  ...
  // Alert debouncing state — see config.scan.alertConfirmMs / alertCooldownMs
  private confirmedAlertSymbol = "__init__"; // the Primary symbol we last actually alerted on
  private candidateAlertSymbol = "__init__"; // the Primary symbol currently "trying" to become confirmed
  ...
  async start() {
    const symbols = await this.selectUniverseWithRetry();
  ...
  /**
   * Wraps the initial selectUniverse() call (used only on boot) with its own retry loop, on top
   * of the per-request retries already inside restClient.ts. This is what actually stops a
   * Binance outage from crashing the whole process: even if every individual request's retries
   * are exhausted, this keeps trying every 30s (capped) rather than throwing — the scanner
   * simply stays "not started yet" and the dashboard shows its normal "waiting for first scan"
   * state until Binance recovers, instead of the server going down entirely.
   */
  private async selectUniverseWithRetry(): Promise<string[]> {
    let attempt = 0;
    while (true) {
      try {
        return await this.selectUniverse();
      } catch (err) {
        attempt++;
        const delay = Math.min(30000, 2000 * 2 ** attempt);
        console.error(
          `[scanner] Failed to build symbol universe (attempt ${attempt}) — retrying in ${delay / 1000}s.`,
  ...
  private async runCycle() {
    ...
    // `markPrices` is fed by a separate live tick stream (@markPrice@1s) that can lag behind
    // the candle data for a given symbol, especially right after that symbol first becomes the
    // Primary trade. Previously the CEO used markPrices directly and silently fell back to 0
    // when a tick hadn't arrived yet — which showed up as "Entry: 0.0000" (and wrong SL/TP1/TP2,
    // since those are computed relative to entry) in the app and in alerts. This builds a
    // merged map that always has a real, recent number: the live tick price when available,
    // otherwise the latest 5m candle close for that symbol (which we know is reliably arriving,
    // since it's what trend/liquidity/risk scores are already computed from).
    const effectivePrices = new Map<string, number>();
    for (const [symbol, candles] of this.candleBuffers) {
      const liveTick = this.markPrices.get(symbol);
      const lastClose = candles[candles.length - 1]?.close;
      const price = liveTick ?? lastClose;
      if (price) effectivePrices.set(symbol, price);
    }
    ...
    updateLatestScanState({ trends, liquidity, risk, rankings: ranked, ceoReport: report });

    // Debounced check: is this a genuinely new, sustained Primary trade — not just a single
    // noisy tick? See evaluateAlertDebounce() docs for why this matters.
    const newPrimarySymbol = report.primary?.symbol ?? "NO_TRADE";
    const isConfirmedChange = this.evaluateAlertDebounce(newPrimarySymbol);

    await this.persistCycleThrottled(scanId, ranked, report, isConfirmedChange);
  ...
  /**
   * Returns true only when the Primary symbol has both (a) held steady for
   * config.scan.alertConfirmMs, marking it as a real change rather than one noisy tick, and
   * (b) respected config.scan.alertCooldownMs since the last alert, so two genuine changes in
   * quick succession still can't spam faster than the cooldown allows.
   *
   * Runs every scan cycle (every ~1s) but only returns true on the single cycle where a change
   * actually gets confirmed — callers should treat this as a one-shot signal, not a status flag.
   */
  private evaluateAlertDebounce(newPrimarySymbol: string): boolean {
    const now = Date.now();

    if (newPrimarySymbol !== this.candidateAlertSymbol) {
      // The candidate changed — restart the stability timer for this new candidate.
      this.candidateAlertSymbol = newPrimarySymbol;
      this.candidateSince = now;
      return false;
    }

    const heldLongEnough = now - this.candidateSince >= config.scan.alertConfirmMs;
    const isActuallyNew = this.candidateAlertSymbol !== this.confirmedAlertSymbol;
    const cooldownElapsed = now - this.lastAlertSentAt >= config.scan.alertCooldownMs;

    if (heldLongEnough && isActuallyNew && cooldownElapsed) {
      this.confirmedAlertSymbol = this.candidateAlertSymbol;
   ...
   */
  private async persistCycleThrottled(
    scanId: string,
    ranked: ReturnType<typeof rankOpportunities>,
    report: CeoReport,
    isConfirmedPrimaryChange: boolean
  ) {
    const dueForPersist = Date.now() - this.lastPersistAt >= config.scan.persistIntervalMs;

    if (!isConfirmedPrimaryChange && !dueForPersist) return;

    this.lastPersistAt = Date.now();
    await this.persistCycle(scanId, ranked, report, isConfirmedPrimaryChange);
  }
  ...
  private async persistCycle(
    ...
    try {
      ...
      >[]) {
        ...
        // Only open a new lifecycle entry on a debounce-confirmed Primary change — using the
        // raw per-cycle comparison here would create a duplicate OPEN row every time the score
        // flickers across the approval threshold, same issue that caused the alert spam.
        if (trade.slot === "PRIMARY" && isConfirmedPrimaryChange) {
          await openLifecycleEntry(client, rows[0].id, trade);
        }
      }
